...
- **`docs.mdc`** — update `README.md`, `CHANGELOG.md`, `AGENTS.md`,
  ...
  docs are created only on the listed triggers, including graduating an
  outgrown README section, first releases, and explicitly requested specs.
  A project's first decision record requires confirmation. The default layout
  puts platform-read files at the repo root and other docs in `docs/`, unless
  the project already has its own convention.
