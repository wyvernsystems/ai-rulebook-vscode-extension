---
description: Test and verification constraints
alwaysApply: true
---

# Tests

- Write the failing tests for the requirement first, then write the code to make them pass.
- Match the project's existing test file location and naming convention (colocated, mirrored `tests/` folder, feature-grouped, etc.). If none exists, ask, or default to colocating the test next to the source file it covers using the ecosystem's standard suffix (e.g. `foo.test.ts`, `foo_test.go`).
